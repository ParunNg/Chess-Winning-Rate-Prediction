...
layout = dbc.Container([
    html.Div([
        html.H3('Chess Winning Rate Prediction', style={"margin-bottom":'20px'})],
    ...
    dbc.Row([
        html.Div([
            ...
            dbc.Input(id="white_rating", type="number", min=1, max=3000, value=1500, placeholder="White Rating", style={"margin-bottom": '20px'}), # 1983 is the minimum year in the cars dataset

            html.H5("Black Rating"),
            dbc.Label("Enter the rating of black player"),
            dbc.Input(id="black_rating", type="number", min=1, max=3000, value=1500, placeholder="Black Rating", style={"margin-bottom": '20px'}),

            html.H5("Start Time Limit"),
            dbc.Label("Enter the start time limit of a chess match (If there is no time limit, leave it as 0)"),
            dbc.Input(id="time_limit", type="number", min=0, max=180, value=0, placeholder="Start Time Limit", style={"margin-bottom": '20px'}),

            html.H5("Increment"),
            dbc.Label("Enter the time increment every time a player ends their turn (If there is no increment, leave it as 0)"),
            dbc.Input(id="increment", type="number", min=0, max=180, value=0, placeholder="Increment", style={"margin-bottom": '20px'}),
            ...
            html.Output(id="win_rate", children="", style={"margin-top": '10px', "font-size": '20px', "font-weight":'bold'})
            ],
            style={"margin-top": "30px"})
        ],
        style={"margin-bottom":'30px', "margin-right":'30px', "display":'inline-block', "width": '700px'})
    ])
], fluid=True)
